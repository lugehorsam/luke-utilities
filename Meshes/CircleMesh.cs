namespace Mesh
{
    ...
    public class CircleMesh : SimpleProceduralMesh
    {
        ...
        public CircleMesh(float radius)
        {
            ...
            {
                _triangles.Add(new TriangleMesh(originVertex, lastVertex, vertex));
        ...
        public static List<Vertex> CreateVertexRing(float radius, Action<Vertex> onVertexCreate = null)
        {
            ...
            for (var i = 0; i < _NUM_VERTEX_ITERATIONS; i++)
            {
                var point = new Vertex(Quaternion.AngleAxis(i * _ANGLE_PER_ITERATION, Vector3.back) * Vector3.up * radius);
                vertices.Add(point);
